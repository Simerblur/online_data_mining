...
class CsvPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, MovieItem):
            ...
            # Map item fields to schema fields (user_score -> metacritic_user_score)
            data = adapter.asdict()
            row = {k: data.get(k) for k in self.schemas["metacritic_data"]}
            row["metacritic_user_score"] = data.get("user_score")
    ...
    def process_item(self, item, spider):
        try:
            ...
            if isinstance(item, MovieItem):
                ...
                # Map item fields to DB schema (user_score -> metacritic_user_score)
                data = adapter.asdict()
                row = {
                    "movie_id": data.get("movie_id"),
                    "metacritic_slug": data.get("metacritic_slug"),
                    "metascore": data.get("metascore"),
                    "metacritic_user_score": data.get("user_score"),
                    "critic_review_count": data.get("critic_review_count"),
                    "user_rating_count": data.get("user_rating_count"),
                    "scraped_at": data.get("scraped_at"),
                }
                self._insert("metacritic_data", row, replace=True)
            elif isinstance(item, MetacriticCriticReviewItem):
