...
class MetacriticSpider(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if imdb_db_path:
            ...
            self.imdb_db_path = Path("output/movies.db")
            if not self.imdb_db_path.exists():
                # Fallback implementation for nested directory structure
                self.imdb_db_path = Path(__file__).parent.parent.parent / "output" / "movies.db"
    ...
    def start_requests(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT movie_id, title, year FROM movie ORDER BY movie_id LIMIT ?", (self.max_movies,))
    ...
    def parse_movie(self, response):
        ...
        for p in range(self.max_review_pages):
            ...
            if p > 0:
                url += f"?page={p}"
            yield scrapy.Request(url=url, callback=self.parse_critic_reviews, meta={"movie_id": movie_id})

        for p in range(self.max_review_pages):
            url = f"https://www.metacritic.com/movie/{slug}/user-reviews/"
            if p > 0:
                url += f"?page={p}"
            yield scrapy.Request(url=url, callback=self.parse_user_reviews, meta={"movie_id": movie_id})
    ...
    def _extract_review_count(self, text: str, kind: str) -> Optional[int]:
        pattern = r"\bBased on\s+([\d,]+)\s+Critic Reviews\b" if kind == "critic" else r"\bBased on\s+([\d,]+)\s+User Ratings\b"
